function site = fmcw_deformation(filename1,filename2,cfgi)
...
% Based on fmcw_melt by
% Craig Stewart, 2014

global cfg

%% Load processing settings
cfg = fmcw_config_deformation; % Load default configuration
if nargin >2
    % overwrite defaults
...
[f.rangeCoarse,~,f.specCor,f.specRaw] = fmcw_range(fm,cfg.p,cfg.maxRange,cfg.winFun); % note - should this be changed to a weighted mean for cases where the attenuation changes within the burst
gm = fmcw_burst_mean(g);
%[g.rangeCoarse,g.rangeFine,g.specCor,g.specRaw] = fmcw_range(gm,cfg.p,cfg.maxRange,cfg.winFun);
[g.rangeCoarse,~,g.specCor,g.specRaw] = fmcw_range(gm,cfg.p,cfg.maxRange,cfg.winFun);
...
if cfg.doAlignBed
    ...
    f.range = f.rangeCoarse; % total range shot 1
    g.range = g.rangeCoarse; % total range shot 2
    ...
    if abs(bnDiff*dr)<1
        g.specRawUnshifted = g.specRaw; % keep a copy of g.specCor for plotting
...
AC.stepSizeM = cfg.coarseChunkWidth/2;
...
AC.isGood = AC.ampCor>=cfg.minAmpCor & AC.ampCorProm>=cfg.minAmpCorProm& AC.range > 150 & AC.range < 300; % Falk 2023: Included range limits here, e.g.: & AC.range > 150 & AC.range < 300;
...
%            peaklag = round(AC.dhInterp/dr); % bin lag
    ...
    case 'xcor'
        ...
        for ii = 1:numel(binStart)
            ...
            [~,AF.mcialt(ii)] = max(AF.AMPCOR(ii,:)); % use best lag from fine cor

            AF.ampCor_alt(ii) = AF.AMPCOR(ii,AF.mcialt(ii));
            AF.cor_alt(ii) = AF.COR(ii,AF.mcialt(ii)); % complex correlation at best amp correlation point
            % will be overwritten later...
            range_alt(ii) = AF.RANGE(ii,AF.mcialt(ii)); % bin centre range (weighted by amplitude of f.specCor.*g.specCor)
            % will be overwritten later...
...
%             if ~cfg.doBulkAllignment
                ...
                si = max(AF.ampCor(range>cfg.firnDepth)); % index of best correlation point
                si = find(AF.ampCor==si);
...
%             end
            ...
            for ii = si+1:length(AF.mci)
                ...
                mci_pmu_mean = round(mean(mci_pmu(max(si, ii-5):ii-1)));
                [~,bpki] = min(abs(mci_pmu_mean-pki)); % closest minimum to last
                mci_pmu(ii) = pki(bpki);
            ...
            for ii = si-1:-1:1
                [~,pki] = findpeaks(-abs(angle(AF.COR(ii,:)))); % index of phase difference minimum
                mci_pmu_mean = round(mean(mci_pmu(min(si, ii+5):-1:ii+1)));
                [~,bpki] = min(abs(mci_pmu_mean-pki)); % closest minimum to last
                mci_pmu(ii) = pki(bpki);
                %mci_pmu(ii) = mci_pmu(ii-1) + round(angle(c(mci_pm(ii-1),ii))/dphi); % lag to give smallest phase diff
            end
...
sr_pt.gi = find(range>=cfg.minDepth & ...                %dhe<1 & ...
...
if cfg.doStrainFit
    nIntervals = numel(cfg.fitIntervals) - 1;

    sr_fit.range = range;
    sr_fit.gi = zeros(size(range));
    sr_fit.midRange = zeros(nIntervals,1);
    sr_fit.M  = zeros(nIntervals,2);
    sr_fit.Me = zeros(nIntervals,2);
    sr_fit.vsn = zeros(nIntervals,1);
    sr_fit.vsre = zeros(nIntervals,1);

    for n = 1:nIntervals
        gi_section = (range>=cfg.fitIntervals(n) & range<cfg.fitIntervals(n+1));
        sr_fit.gi(gi_section) = n;
            % Loop through fitting to different depth ranges
        if length(gi_section) < cfg.minPointsToFit
            sr_fit.M(n,:)  = nan;
            sr_fit.Me(n,:)  = nan;
            disp(['warning: not performing strain fitting in range ' int2str(cfg.fitIntervals(n)) ' to ' int2str(cfg.fitIntervals(n+1)) ', because of to little points to fit.'])
            error('Not enough good correlations in depth range')
        else
            %setup things to fit here - i.e. offset, linear and optionally bending
            sr_fit.midRange(n) = mean(range(gi_section)); % mean range used in fit
            switch cfg.fit.type
                case 'linear'
                    predictors = [ones(numel(range(gi_section)),1) transpose(range(gi_section))-sr_fit.midRange(n)];
                    % Note: its important to de-mean range, otherwise errors are wrong.
                case 'quadratic' % for bending
                    predictors = [ones(numel(range(gi_section)),1) (range(gi_section)-sr_fit.midRange(n)).' ((range(gi_section)-sr_fit.midRange(n)).^2).'];
        %             % See Jenkins 2006
        %            not yet implemented - we have to change lots below for this...
            end
            switch cfg.fitMethod
                case 'menke'
                    [sr_fit.M(n,:), sr_fit.Me(n,:)] = menke_fit(predictors,dh(gi_section),dhe(gi_section));% linear fit with error
                case 'robust'
                    [sr_fit.M(n,:), STATS] = robustfit(predictors(:,2:end),dh(gi_section)); % linear fit - downweighting outliers
                    sr_fit.Me(n,:) = STATS.se; % ???
                    ...
                    switch cfg.fit.errorMetric
                        ...
                        case 'alpha'
                            ...
                            sr_fit.alpha = 0.05;
                            sr_fit.Mci = nlparci(sr_fit.M(n,:),STATS.resid,'cov',STATS.covb,'alpha',sr_fit.alpha);
        ...
        sr_fit.M(n,1) = sr_fit.M(n,1)-sr_fit.midRange(n)*sr_fit.M(n,2); % correct for the fact we removed mean range before fit
        sr_fit.vsr(n) = sr_fit.M(n,2)*daysPerYear/dt; % vertical strain rate (per year)
        sr_fit.vsre(n) = sr_fit.Me(n,2)*daysPerYear/dt; % vertical strain rate (per year)
%         sr_fit.dh = sr_fit.M(n,1)+range.*sr_fit.M(n,2); % linear fit to internal strain
...
%     Disp(' ')
    ...
    if cfg.doPlotFit || cfg.doPlotAll
       ...
       for n = 1:nIntervals
           ...
           plotrange = [cfg.fitIntervals(n) sr_fit.midRange(n) cfg.fitIntervals(n+1)];
           plotdhe = sqrt(sr_fit.Me(n,1).^2 + ((plotrange-sr_fit.midRange(n)).*sr_fit.Me(n,2)).^2);
           plotdh = sr_fit.M(n,1)+plotrange.*sr_fit.M(n,2);
       ...
       plot(range,dh,'g.','markerSize',20) % used points
       %h = fillXRange([cfg.firnDepth fit.maxDepth],'facecol',[0.6 0.6 0.6],'facealpha',0.3,'edgecol','none');
