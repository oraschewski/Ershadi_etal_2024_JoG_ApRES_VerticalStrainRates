function vdat = LoadBurstRMB3(Filename, Hdr, Burst, SamplesPerChirp)
...
if fid >= 0
    ...
    while BurstCount <= Burst && burstpointer <= filelength - MaxHeaderLen
        ...
        if ~isempty(searchind)
            try
                searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
                vdat.Nsamples = sscanf(Hdr(searchind(1)+8:searchCR(1)+searchind(1)),'%d');
                WperChirpCycle = vdat.Nsamples + WperChirpHdr;
                searchind = strfind(Hdr, 'Chirps in burst:');
                searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
                vdat.ChirpsInBurst = sscanf(Hdr(searchind(1)+16:searchCR(1)+searchind(1)),'%d');
    ...
    try
        searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
        vdat.Attenuator_1 = sscanf(Hdr(searchind(1)+13:searchCR(1)+searchind(1)),'%f,',4);
    catch err
        vdat.Code = 1;
    end
    ...
    try
        searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
        vdat.Attenuator_2 = sscanf(Hdr(searchind(1)+13:searchCR(1)+searchind(1)),'%f,',4);
    catch err
        vdat.Code = 1;
    end
