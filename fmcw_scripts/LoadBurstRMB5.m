function vdat = LoadBurstRMB5(Filename, Hdr, Burst, SamplingFrequency)
    ...
    while BurstCount <= Burst && burstpointer <= filelength - MaxHeaderLen
        ...
        if ~isempty(searchind)
            try
                ...
                searchind = strfind(Hdr, SearchString);
                searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
                vdat.TxAnt = sscanf(Hdr(searchind(1)+length(SearchString):searchCR(1)+searchind(1)),'%d',8);
                
                SearchString = 'RxAnt=';
                searchind = strfind(Hdr, SearchString);
                searchCR = strfind(Hdr(searchind(1):end),[char(13),char(10)]);
                vdat.RxAnt = sscanf(Hdr(searchind(1)+length(SearchString):searchCR(1)+searchind(1)),'%d',8);
