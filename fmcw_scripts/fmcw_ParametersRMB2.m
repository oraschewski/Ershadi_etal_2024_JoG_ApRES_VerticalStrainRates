function H = fmcw_ParametersRMB2(dataFile,Hdr)
...
loc1 = strfind(Hdr,'Reg0');
loc2 = strfind(Hdr,'="');

for k = 1:length(loc1)
    switch(Hdr(loc1(k):loc2(k)))
        case 'Reg01=' %Control Function Register 2 (CFR2)—Address 0x01 Four bytes
            ...
            loc3 = strfind(Hdr(loc2(k)+2:end),'"');
            val = Hdr((loc2(k)+2:loc2(k)+loc3(1)));
...
loc = strfind(Hdr,'SamplingFreqMode=');
if ~isempty(loc)
    searchCR = strfind(Hdr(loc(1):end),[char(10)]);
    H.fs = sscanf(Hdr(loc(1)+length(['SamplingFreqMode=']):searchCR(1)+loc(1)),'%d\n');
    if H.fs == 1
        H.fs = 8e4;
    else
        H.fs = 4e4;
...
loc = strfind(Hdr,'N_ADC_SAMPLES=');
searchCR = strfind(Hdr(loc(1):end),[char(10)]);
H.Nsamples = sscanf(Hdr(loc(1)+length(['N_ADC_SAMPLES=']):searchCR(1)+loc(1)),'%d\n');
