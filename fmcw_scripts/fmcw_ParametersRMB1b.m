function H = fmcw_ParametersRMB1b(filename,Hdr)
...
loc = strfind(Hdr,'Samples: ');
searchCR = strfind(Hdr(loc(1):end),[char(10)]);
H.Nsamples = sscanf(Hdr(loc(1)+length(['Samples: ']):searchCR(1)+loc(1)),'%d\n');
...
fid = fopen(strcat(path,filesep,'Config.ini'),'rt'); % Compatible with Linux
if fid==-1
    fid = fopen(strcat(path,filesep,'Config.ini'),'rt'); % In case file renamed to lowercase
end
