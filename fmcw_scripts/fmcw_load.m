...
if strcmp(ext,'.mat')
    ...
    [FileFormat,Hdr] = fmcw_file_format(filename);
    if FileFormat == 6
        vdat = LoadBurstRMB6(filename, Hdr, burst);
    elseif FileFormat == 5
        vdat = LoadBurstRMB5(filename, Hdr, burst, SamplingFrequency);% Data from after Oct 2014 (RMB2b + VAB Iss C, SW Issue >= 101)
    elseif FileFormat == 4
        vdat = LoadBurstRMB4(filename, Hdr, burst, SamplingFrequency); % Data from after Oct 2013  (RMB1b)
    elseif FileFormat == 3
        vdat = LoadBurstRMB3(filename, Hdr, burst, SamplingFrequency); % Data from Jan 2013 (RMB1a)
    elseif FileFormat == 2
        %vdat = LoadOldBurstCompat(filename, burst, SamplingFrequency); % Data from Prototype FMCW radar (nov 2012) (RMBA?)
        vdat = LoadBurstMcM(filename, Hdr, burst);
    end
end
...
if ~ischar(FileFormat)
    ...
    if FileFormat == 5
        H = fmcw_ParametersRMB2(vdat.filename,vdat.Header);
    elseif FileFormat == 4
        H = fmcw_ParametersRMB1b(vdat.filename,vdat.Header);
    end
